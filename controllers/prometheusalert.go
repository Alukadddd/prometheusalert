	...
	if pMsg.Type != "" && PrometheusAlertTpl != nil {
		...
		if pMsg.Split != "false" && PrometheusAlertTpl.Tpluse == "Prometheus" {
			...
			for _, AlertValue := range Alerts_Value {
				...
				Return_pMsgs := AlertRouterSet(xalert, pMsg, PrometheusAlertTpl.Tplname, PrometheusAlertTpl.TplParam,PrometheusAlertTpl.TplCode,mode)
				if len(Return_pMsgs) == 0 {
					message = "路由未找到!"
				}
				...
				for _, Return_pMsg := range Return_pMsgs {
					...
					//获取渲染后的模版
					err, msg := TransformAlertMessage(p_alertmanager_json, Return_pMsg.TplParam)
			...
			//获取渲染后的模版
			err, msg := TransformAlertMessage(p_json, PrometheusAlertTpl.TplParam)
...
//路由处理
func AlertRouterSet(xalert map[string]interface{}, PMsg PrometheusAlertMsg,TplName, Tpl,TplCode,mode string) []PrometheusAlertMsg {
	return_Msgs := []PrometheusAlertMsg{}
	//原有的参数不变
	if mode == "test" {
		PMsg.TplName = TplName
		PMsg.TplParam = Tpl
		PMsg.TplCode = TplCode
	...
	for _, router_value := range GlobalAlertRouter {
		...
		if rules_num == rules_num_match {
			if PMsg.TplName != "" && PMsg.TplName == router_value.Tpl.Tplname{
				continue
			}
			PMsg.Type = router_value.Tpl.Tpltype
			PMsg.TplName = router_value.Tpl.Tplname
			PMsg.TplParam = router_value.Tpl.TplParam
			PMsg.TplCode = router_value.Tpl.TplCode
...
func SendMessagePrometheusAlert(message string, pmsg *PrometheusAlertMsg, logsign string) string {
	...
	//阿里云短信
	case "alydx":
		ReturnMsg += PostALYmessage(message, pmsg.Phone,pmsg.TplCode, logsign)
	//腾讯云电话
	case "txdh":
